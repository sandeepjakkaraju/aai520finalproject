...
# In this project, we will build a chatbot using conversations from Cornell University's [Movie Dialogue Corpus](https://www.kaggle.com/datasets/rajathmc/cornell-moviedialog-corpus). The main features of our model are LSTM cells, a bidirectional dynamic RNN, and decoders with attention. 
# 
...
# Most of the code to load the data is courtesy of https://www.kaggle.com/datasets/rajathmc/cornell-moviedialog-corpus.
...
def encoding_layer(rnn_inputs, rnn_size, num_layers, keep_prob, sequence_length):
    ...
    lstm = tf.keras.layers.BasicLSTMCell(rnn_size)
    drop = tf.keras.layers.DropoutWrapper(lstm, input_keep_prob = keep_prob)
    enc_cell = tf.keras.layers.MultiRNNCell([drop] * num_layers)
...
with tf.name_scope("optimization"):
    # Loss function
    cost = tf.keras.layers.seq2seq.sequence_loss(
        train_logits,
        targets,
